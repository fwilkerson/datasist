...
const ctx = require('../')('data/query_data');

test('query functionality?', t => {
  const repo = ctx.file('user');

  Promise.all([
    repo.append({ first: 'Frank', last: 'Wilkerson' }),
    repo.append({ first: 'John', last: 'Smith' }),
    repo.append({ first: 'Jane', last: 'Doe' }),
    repo.append({ first: 'Sarah', last: 'Chavez' }),
    repo.append({ first: 'Laura', last: 'Bemos' })
  ])
    .then(() => {
      repo
        .query()
...
test.onFinish(() => {
  const user = resolve('./data/query_data/user.json');
